...
3. **No Centralization:** You evaluate the raw output of agents and tools. You do not write Python wrappers to fix their mistakes. If an agent fails to output an `A2A_HANDOFF` block correctly, the agent's persona must be refined, not the python code.
